...
public static getForgotPasswordMessage(props: VerificationMessageProps) {
    ...
    public static getInvitedUserRegisterMessage(props: InvitedUserRegisterProps): MessageModel {
        return new MessageModel({
            isInternal: false,
            ...
            body:
            `<p>Hello ${props.user.fullName?.length ? props.user.fullName : props.user.email}</p>
            <p>You have been invited to I-Stem portal by your university ${props.universityName}</p>
            <p>Please click <a href=${props.link}>here</a> to create your account</p>
            <p>Team I-Stem</p>
            `
        })
    }

    public static getNewOrganizationRegistrationRequestMessage(props:OrganizationRegistrationRequestProps) {
        return new MessageModel({
            isInternal: false,
            ...
            <p>Click <a href="${props.approvalLink}">here</a> to approve the registration request</p>
            <p>Click <a href="${props.rejectionLink}">here</a> to deny the request if university is duplicate.</p>
            `
        })
    }
...
<p>Team I-Stem</p>
    ...
    public static getNewOrganizationRegistrationRequestRejectionMessage(props:UserDetails) {
        return new MessageModel({
            isInternal:false,
            label: MessageLabel.INVITATION,
