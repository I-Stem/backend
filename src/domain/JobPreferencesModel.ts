...
persistJobPreferences(currUserId: string) {
    const logger = loggerFactory(JobPreferencesModel.ServiceName, 'persistJobPreferences');
    this.userId = currUserId;
    new JobPreferencesDbModel(this)
.save((err: any) => {
    if (err) {
    logger.error(err);
    }
...
public static async jobPreferencesForUser(userId: string): Promise<JobPreferencesModel[]> {
    const logger = loggerFactory(JobPreferencesModel.ServiceName, 'jobPreferencesForUser');
    logger.info(`Job Preferences by user: ${userId}`);
    return JobPreferencesDbModel.find({userId: userId}).exec();
}
}
