...
constructor(props: VCProps) {
    ...
    this.statusLog = props.statusLog || [new VCRequestLifecycleEvent(props.status)];
...
async persist(): Promise<boolean> {
    ...
    try {
    const vcSaved = await (await new VcDbModel(this).save()).execPopulate();
...
public async changeStatusTo(newStatus: VCRequestStatus) {
    ...
    logger.info(`changing status to ${newStatus} for vc Request: ${this.vcRequestId}`);
    this.status = newStatus;
    const event = new VCRequestLifecycleEvent(newStatus);
    this.statusLog?.push(event);

    await VcDbModel.findByIdAndUpdate(this.vcRequestId, {status: newStatus,
    $push: {statusLog: event}});

    return this;
    }

    public async updateOutputURLAndVideoLength(outputURL: string, videoLength: number) {
...
await VcDbModel.findByIdAndUpdate(this.vcRequestId, {
    ...
    public async notifyUserForResults() {
        const logger = loggerFactory(VcModel.serviceName, 'notifyUserForResults');
    ...
    public static async saveReview(vcRequestId: string, review: ReviewModel): Promise<VcModel | null> {
...
return null;
    ...
    public static getVcRequestCountForUser(userId: string): Promise<number> {
        const logger = loggerFactory(VcModel.serviceName, 'getVcRequestCountForUser');
        return VcDbModel.countDocuments({userId: userId}).exec();
    }

    public static async getCustomModelVcRequestCount(userId: string): Promise<number> {
        return VcDbModel.countDocuments({userId: userId, modelName: {$ne: 'standard-@'}}).exec();
    }
