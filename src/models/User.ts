...
UserSchema.pre<IUserModel>('save', async function(_next)  {
    ...
    _verifyUserExpires = new Date(_verifyUserExpires.setDate(
        _verifyUserExpires.getDate() + 1
    ));
...
UserSchema.methods.generatePassword =  function (_requestPassword: string): string {
    const saltRounds = 10;
...
UserSchema.methods.comparePassword = function (_requestPassword: string, _cb: any): any {
	bcrypt.compare(_requestPassword, this.password, (_err: any, _isMatch: boolean) => {
...
UserSchema.methods.generateResetToken = function (_passwordResetExpires: Date): any {
...
UserSchema.methods.generateResetExpiryDate = function () {
    ...
    _passwordResetExpires = new Date(_passwordResetExpires.setHours(
        _passwordResetExpires.getHours() + 2
    ));
    return _passwordResetExpires;
};

UserSchema.methods.addCredits = function(_credits: number, _reason: string): void {
...
UserSchema.methods.deductCredits = function(_credits: number, _reason: string): void {
...
UserSchema.methods.checkCredits = async function(_credits: number): Promise<any> {
    ...
    return await Ledger.aggregate([
        ...
        {$project: {
            checkCredits: {
                $cond: [{ $gte: [{ $subtract: ['$credits', '$debits'] }, _credits] }, true, false]
            }
        }}
    ]).exec();
};
