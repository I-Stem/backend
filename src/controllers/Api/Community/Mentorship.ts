...
class MentorshipController {
    ...
    public static async addMentorship(req: Request, res: Response) {
        const logger = loggerFactory(
            MentorshipController.ServiceName,
            "addMentorship"
        );
        ...
        if (req.body.mentorshipId) {
            ...
            if (req.body.mentorshipStatus.length !== 0)
                await MentorshipModel.updateMentorshipForUser(
                    req.body.mentorshipId,
                    req.body.mentorshipStatus,
                    req.body.signupAs
                );
            if (req.body.cancelMenteeship !== undefined)
                await MentorshipModel.updateMenteeshipForUser(
                    req.body.mentorshipId,
                    req.body.cancelMenteeship,
                    req.body.signupAs
                );
            emailService.reportMentorship(
                MentorshipTemplate.getMentorshipMessage({
        ...
        if (user !== null) {
            if (
                ...
                emailService.sendEmailToUser(
                    user,
                    MentorshipTemplate.getMentorshipApplicationReceivedAcknowledgementMessage(
                        {
                            user: user,
                            subject:
                                "[I-Stem] Thanks for signing up as mentee!!",
                    ...
                    MentorshipTemplate.getMentorshipApplicationReceivedAcknowledgementMessage(
                        {
                            user: user,
                            subject:
                                "[I-Stem] Thanks for signing up as mentor!!",
...
`,
    ...
    public static async get(req: Request, res: Response) {
        ...
        try {
            const results = await MentorshipModel.MentorshipForUser(
                res.locals.user.id
            );
