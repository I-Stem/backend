...
class VCController {
        ...
        try {
            ...
            if (vcRequest !== null && sourceFile !== null) {
                ...
                LedgerModel.createDebitTransaction(
                    res.locals.user.id,
                    Math.ceil(vcRequest.videoLength / 60) * 25,
                    "Escalation request for file: " + vcRequest?.documentName
                );
    ...
    public static async vcCallback(req: Request, res: Response) {
        ...
        try {
            const file = await FileModel.getFileByExternalVideoId(
