...
class PassportStrategies {
    ...
    public static getGoogleStrategy() {
        const logger = loggerFactory(
            PassportStrategies.servicename,
            "getGoogleStrategy"
        );
        ...
        return new GoogleStrategy.Strategy(
            ...
            async (req, token, refreshToken, profile, done) => {
                ...
                if (userData.invitationEmail || userData.invitationToken) {
                    if (
                        userData.invitationEmail?.toLowerCase() !==
                        profile._json.email.toString().toLowerCase()
                    ) {
                        done(
                            new InvitationEmailMismatchError(
                                "Invitation email: " +
                                    userData.invitationEmail +
                                    " doesn't match with provided email: " +
                                    profile._json.email
                            ),
                            undefined
                        );
                ...
                if (user) {
                    ...
                    userData.authFlow?.toString()?.toLowerCase() === "login"
                ) {
                    logger.info(
                        "trying to signin with nonexisting email: " +
                            profile._json.email
                    );
                    done(
                        new NoSuchUserError(
                            `No account exist with provided email id: ${profile._json.email}, Please create an account first.`
                        ),
                        undefined
                    );
                } else {
                    try {
                        const userType = getUserTypeFromString(
                            userData.userType
                        );
                        const newUser = await UserModel.registerUser(
                            ...
                            userData.invitationToken?.toString(),
                            undefined,
                            userData.invitationType
                        );
